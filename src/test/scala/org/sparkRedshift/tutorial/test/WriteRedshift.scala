...
class WriteRedshift extends Matchers with ShouldVerb with ScalaFutures with EN with ScalaDsl with AcceptanceTestFilesUtils with AcceptanceTestRedshiftUtils{
  ...
  Then("""^using RedShiftConnector I copy "([^"]*)" table to "([^"]*)"  and check that has (\d+) records$"""){
    (tableName:String, testTableName:String, expectedResult:Int) =>
      ...
      RedShiftConnectorImpl.createTempTable(categoryDF, "categoryTmp")
      RedShiftConnectorImpl.writeTable(testTableName,SaveMode.Overwrite,s"SELECT * FROM categoryTmp", None)
      val amount = RedShiftConnectorImpl.readTable(Some(testTableName),None).count()
  ...
  Then("""^using RedShiftConnector I append "([^"]*)" table to "([^"]*)"  and check that has (\d+) records$"""){
    (tmpTableName:String, testTableName:String, expectedResult:Int) =>
      RedShiftConnectorImpl.writeTable(testTableName,SaveMode.Append,s"SELECT * FROM categoryTmp", None)
      val amount = RedShiftConnectorImpl.readTable(Some(testTableName),None).count()
      assertTrue(s"temp $testTableName Table should have $expectedResult  but has $amount records ", amount == expectedResult)
  }
  ...
  Then("""^using RedShiftConnector I make a write statement with saveMode "([^"]*)" over  "([^"]*)" table to "([^"]*)"  and check that has (\d+) records$"""){
    (saveMode:String, tmpTableName:String, testTableName:String, expectedResult:Int) =>
      ...
      RedShiftConnectorImpl.writeTable(testTableName,sMode,s"SELECT * FROM categoryTmp", None)
      val amount = RedShiftConnectorImpl.readTable(Some(testTableName),None).count()
      assertTrue(s"temp $testTableName Table should have $expectedResult  but has $amount records ", amount == expectedResult)
  }
  ...
  After("@cleanTestCategoryTable") { scenario: Scenario =>
    ...
    RedShiftConnectorImpl.dropTempTable("categoryTmp")
  }
